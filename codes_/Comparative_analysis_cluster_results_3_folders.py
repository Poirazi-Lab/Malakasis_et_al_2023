...
full_path_data_2 ="/media/nikos/Elements/ClusterResults/results_301121_AllDigits_20runs_8280interstim_random_turnover_alternating_presentation/"
full_path_data_3 ="/media/nikos/Elements/ClusterResults/results_021221_AllDigits_20runs_8280interstim_no_turnover_alternating_presentation_200_iters/"
...
for full_path_data in [full_path_data_1,full_path_data_2,full_path_data_3]:
	accs_list=[]
	for d2 in range(10):
		for d1 in range(d2):	
...
for d2 in range(10):
	for d1 in range(d2):
		...
		ax1.set_xticklabels(['Constrained Turnover','Random Turnover',"No Turnover"],
		...
		for p in pvals:
			...
			if p<0.05:
				#plt.show()
				plt.savefig("./temp/accuracy_comparison_turnovers_"+str(d1)+"_and_"+str(d2))
				break
