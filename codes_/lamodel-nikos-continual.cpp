...
void RunPattern(LANetwork& net,  vector<double> pat, double label, int active_nrn_spike_thresh, float max_inp_freq, float label_inp_freq, bool Train)
{
	...
	{
		LANeuron* n = *ni;
		if (float(n->total_spikes) > active_nrn_spike_thresh )// if total spikes exceed 20(I think) the neuron is considered active
		{
			tActive++;
			//Draft
...
void RunSNN(LANetwork& net, int TOT_PAT_TRAIN, int TOT_PAT_TEST , string suffix, int EPOCHS, int turnover_iter, float rewired_perc, float turnover_thresh, int active_neuron_spike_thresh,
 float max_input_freq, float label_input_freq, float stop_thresh, float wmin, float wmax, int class_1, int class_2)
{
...
//***********************TEST************************
    ...
    vector<double> test_labelsvec(number_of_images_test);//contains the 10000 MNIST testing image labels(prob don't need it for now)
...
//******************************************************
    ...
    int sample_size=20;//sample testing batch size
    ...
    int continualswitch_2 = 0; //set to 1 if lr_thresh condition is surpassed
    float cont_thresh = 0.25;
    int cont_class=class_1; //variable regarding current continual learning class
    int breakpoint= 500;// When to stop training
	...
	for (j=0; j < EPOCHS; j++)
	{
		...
		for (i=0; i < TOT_PAT_TRAIN; i++)
		{
			...
			//***********SETTING UP CONTINUAL LEARINING IN HERE. *****************
			
			if (train_labelsvec[indexes[i]] != cont_class)
			{
				continue;
			}
			//*********************************************************************
...
//********************************************************************************************************************************************************************
			...
			*/
			...
			if (k==breakpoint)
			{
				cout<<"REACHED BREAKPOINT, TESTING NOW"<<endl;
				break;
			}
			...
			if (lr_perc >= cont_thresh && continualswitch_2==0)
			{	
				purged = net.PurgeInputSynapsesNikos(0.5);
				...
				cout<<"I LEARNED DIGIT 1, GIVE ME DIGIT 2!!!!!!"<<endl;
...
int main( int argc, char* argv[])
{
	...
	float pattern_to_hidden_synapses= 3500;
	float label_to_hidden_synapses = 2;
	float lateral_inh_synapses = 2;
	float Wmin = 0.1;
	float Wmax = 0.2;
	int iterations_to_turnover = 10;
	float rewired_percentile = 1;
	float turnover_threshold = 0;
	int active_neuron_spike_threshold = 20;
	float max_input_frequency = 30;
	float label_input_frequency= 30;
	...
	RunSNN(net, numPatternsTrain, numPatternsTest, suffix, epochs, iterations_to_turnover, rewired_percentile, turnover_threshold,
	 active_neuron_spike_threshold, max_input_frequency, label_input_frequency, stop_threshold, Wmin, Wmax, digit1, digit2); 
	return 0;
