...
// Returns the (positive/LTP or negative/LTD) magnitude of the synaptic tags depending on the  Calcium level ca (NIKOS: CHANGE 1.3 TO 2 AND 0.3 TO 1 TO GET SYMMETRIC LTP AND LTD)
inline float caDP(float ca)
{


	float f = (2.0/(1.+exp(-(ca*10.-3.5)*10.))) - (1.0/(1.+exp(-(ca*10.0-2.0)*19.)));
	return f;
}
	...
	for (int i =0 ;i < number; i++)
	{
		...
		else // Type = normal neuron that has branches and all
		{
			...
			for (int tt =0; tt < n_branches_per_neuron; tt++)
			{
				...
				{	
					if (nlTypePYR == DEND_MIXED)
					{
						if (tt < 0.5*n_branches_per_neuron) bb->nlType = DEND_SUB;
						else bb->nlType = DEND_SUPRA;
					}
					else if (nlTypePYR < DEND_MIXED && nlTypePYR >= 0)
						bb->nlType = nlTypePYR;
					else
					{
						printf("bad type %d", nlTypePYR);
						abort();
					}
				}
				else if ( type == 'M') // SOM interneurons
				{
					...
					else if (nlTypeSOM < DEND_MIXED && nlTypeSOM >= 0)
						bb->nlType = nlTypeSOM;
					else
					{
...
int LANetwork::ConnectNeurons(vector<LANeuron*> fromList, vector<LANeuron*> toList, bool isClustered,  int nNeuronPairs, int nSynapsesPerNeuron, float initialWeightMin = -1, float initialWeightMax = -1, bool fullCon=false, bool isPlastic= false)
{
	...
	while(true)
	{
		...
		if (fullCon)
		{
			...
			while(toList2.at(toIdx)->nid == a->nid)
			{
				toIdx = int(rgen()*(float)toList2.size());
			}
		...
		else
		{
			...
			while(toList.at(toIdx)->nid == a->nid)
			{
				toIdx = int(rgen()*(float)toList2.size());
			}
...
int LANetwork::PurgeInputSynapsesInSubpopNikos(float weightLimit, vector<LANeuron*> subpop) // NIKOS: Changing inputs_cs with my pattern_inputs vector, since that one is not usable
{
	int totalFound =0;
	for (nrn_iter ni = this->pattern_inputs.begin(); ni != this->pattern_inputs.end(); ni++)
	{
		...
		if (n->outgoing.size())
		{
			...
			while (si != n->outgoing.end())
			{
				if (find(subpop.begin(),subpop.end(),(*si)->target_nrn) != subpop.end())
				{
					LASynapse* s = *si;
	...
	cout << "**********" << totalFound <<  " Synapses removed in this Subpopulation."  << "**********" << endl;  
...
int LANetwork::PurgeRecurrentSynapsesNikos(float weightLimit) // NIKOS: Changing inputs_cs with my pattern_inputs vector, since that one is not usable
{
	int totalFound =0;
	for (nrn_iter ni = this->pyr_list.begin(); ni != this->pyr_list.end(); ni++)
	{
		...
		if (n->outgoing.size())
		{
			...
			{
				LASynapse* s = *si;
				if (s->weight <= weightLimit && s->isPlastic)
				{
					VEC_REMOVE(s->source_nrn->outgoing, s);
					VEC_REMOVE(s->target_nrn->incoming, s);
					VEC_REMOVE(s->target_branch->synapses, s);
	...
	cout << "**********" << totalFound <<  " Recurrent Synapses removed."  << "**********" << endl;  


	return totalFound;
}

int LANetwork::PurgeRecurrentSynapsesInSubpopNikos(float weightLimit, vector<LANeuron*> subpop) // NIKOS: Changing inputs_cs with my pattern_inputs vector, since that one is not usable
{
	int totalFound =0;
	for (nrn_iter ni = subpop.begin(); ni != subpop.end(); ni++)
	{
		LANeuron* n = *ni;
		if (n->outgoing.size())
		{
	...
	cout << "**********" << totalFound <<  " Recurrent Synapses removed in this Subpopulation."  << "**********" << endl;  
...
//diff_syns -> synapses from label input to the other subpopulation for difficulty increase.

void LANetwork::CreateFearNet(int nneurons, int nbranches, float inh_ctrl_syns, float pat_to_hid_syns, float lab_to_hid_syns, float diff_syns,
 ...
 float lat_inh_syns, float initWmin, float initWmax, int class_1, int class_2)
{
	...
	/* 
	this->CreateNeurons(this->n_neurons*0.8*0.1, this->n_branches_per_neuron, 'P', &this->pyr_list_2, &this->pyr_list, 2);
	this->CreateNeurons(this->n_neurons*0.8*0.1, this->n_branches_per_neuron, 'P', &this->pyr_list_3, &this->pyr_list, 3);
	this->CreateNeurons(this->n_neurons*0.8*0.1, this->n_branches_per_neuron, 'P', &this->pyr_list_4, &this->pyr_list, 4);
	this->CreateNeurons(this->n_neurons*0.8*0.1, this->n_branches_per_neuron, 'P', &this->pyr_list_5, &this->pyr_list, 5);
	this->CreateNeurons(this->n_neurons*0.8*0.1, this->n_branches_per_neuron, 'P', &this->pyr_list_6, &this->pyr_list, 6);
	this->CreateNeurons(this->n_neurons*0.8*0.1, this->n_branches_per_neuron, 'P', &this->pyr_list_7, &this->pyr_list, 7);
	this->CreateNeurons(this->n_neurons*0.8*0.1, this->n_branches_per_neuron, 'P', &this->pyr_list_8, &this->pyr_list, 8);
	this->CreateNeurons(this->n_neurons*0.8*0.1, this->n_branches_per_neuron, 'P', &this->pyr_list_9, &this->pyr_list, 9);
	*/
	...
	/* 
	EXPERIMENTING WITH LATERAL INHIBITION PT1
	*/
	...
	// Create input stimulation neurons that bring in the patterns  CHANGING THAT TO 784 FOR MNIST *INPUT DIM*
	CreateNeurons(400, 0, 'S', &this->pattern_inputs);
	...
	//BINARY CLASSIFICATION EXCLUTIONS
	/* 
	CreateNeurons(100, 0, 'S', &this->label_inputs_2);
	CreateNeurons(100, 0, 'S', &this->label_inputs_3);
	CreateNeurons(100, 0, 'S', &this->label_inputs_4);
	CreateNeurons(100, 0, 'S', &this->label_inputs_5);
	CreateNeurons(100, 0, 'S', &this->label_inputs_6);
	CreateNeurons(100, 0, 'S', &this->label_inputs_7);
	CreateNeurons(100, 0, 'S', &this->label_inputs_8);
	CreateNeurons(100, 0, 'S', &this->label_inputs_9);
	*/
	...
	ConnectNeurons(this->in_som, this->pyr_list, 0,  0.1*4*inh_ctrl_syns, 1, -1, .3, false, false);//200
	...
	float reccurent_syns=0;

	if (reccurent_syns>0)
	{
		//this->ConnectNeurons(this->pyr_list, this->pyr_list, 0, n_neurons*0.8*reccurent_syns, 1, initWmin, initWmax , false, true);
		
		this->ConnectNeurons(this->pyr_lists[class_1], this->pyr_lists[class_1], 0, 0.5*n_neurons*0.8*reccurent_syns, 1, initWmin, initWmax , false, true);
		this->ConnectNeurons(this->pyr_lists[class_2], this->pyr_lists[class_2], 0, 0.5*n_neurons*0.8*reccurent_syns, 1, initWmin, initWmax , false, true);
	...
	float DiffWeight = 1;
	int DiffSynsPerNeuron=1;
	//Difficulty: Teaching signal to opposite subpopulation
	if (diff_syns>0)
	{
		this->ConnectNeurons(this->label_inputs[class_1], this->pyr_lists[class_2], 0, 0.5*n_neurons*0.8*diff_syns, DiffSynsPerNeuron, -1, DiffWeight , true, false);
		this->ConnectNeurons(this->label_inputs[class_2], this->pyr_lists[class_1], 0, 0.5*n_neurons*0.8*diff_syns, DiffSynsPerNeuron, -1, DiffWeight , true, false);
	}
...
void LANetwork::LoadNet(int nneurons, int nbranches,  int class_1, int class_2, string filename)
{
	this->n_neurons = nneurons;
	this->n_branches_per_neuron = nbranches;
	...
	// Inhibitory populations - SOM+ (10%) 
	this->CreateNeurons(this->n_neurons*0.1, 10 , 'M', &this->in_som); // SOM (For Lat Inh, scaled down to 0.2 of original amount for now)
	...
	// Create input stimulation neurons that bring in the patterns *INPUT DIM*
	CreateNeurons(400, 0, 'S', &this->pattern_inputs);
	...
	vector<vector<double>> loadedsyns = read_csv(filename);

	
	for (unsigned long int i = 0; i < loadedsyns.size(); ++i)
	{
		LANeuron* pre;
		LABranch* postb;
		for (nrn_iter ni = this->neurons.begin(); ni != this->neurons.end(); ++ni)
		{
			if ((*ni)->nid == loadedsyns[i][0])
			{
				pre = *ni;
				break;
			}
		...
		{
			if ((*bi)->bid == loadedsyns[i][1])
			{
				postb = (*bi);
				break;
			}
		}
		this->AddSynapse(pre, postb, loadedsyns[i][2], false);
	}
		...
		for (nrn_iter ni = this->neurons.begin(); ni != this->neurons.end(); ++ni)
		{
			...
			else /// Pyr or interneuron
			{
				...
				else
				{
					...
					// CREB activation changes the adaptation dynamics ORIGINAL CREB ADAPTATION VALUE IS 70, CHANGING IT TO MAKE CREB EFFECTS BIG
					if (this->disableCreb)
				...
				//Threshold for spike -- CREB also changes the threshold for spiking THIS IS OFF BEFORE NIKOS (IF CREB>100 ORIGINAL)
				if ( lastSpikeT[n->nid] < t-2 && n->V > (20.0 - (n->crebLevel>100. ? 2.0 : 0.) ))
				{
					...
					n->total_spikes++;
					n->isSpiking = true;
					n->vspike = 30.0;  // backpropagating spike voltage to add ***************ORIGINAL VALUE=30******************
					n->V = 70.;
				}
				else
				{
		...
		for (nrn_iter ni = this->pyr_list.begin(); ni != this->pyr_list.end(); ni++)
		{
			...
			for (branch_iter bi = nrn->branches.begin(); bi != nrn->branches.end(); ++bi)
			{
				...
				for (syn_iter si = b->synapses.begin(); si != b->synapses.end(); ++si)
				{
					...
					//******************************COMMENT OUT FOR NO HOMEOSTASIS*********************************
			...
			*/
			...
			//CREB decay COMMENT OUT FOR NO DECAY ----> PERMANENT EFFECTS OF CREB DURING TRAINING
