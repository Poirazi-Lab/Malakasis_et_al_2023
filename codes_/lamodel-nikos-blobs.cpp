...
#include "constructs-nikos-blobs.h"
#include <iostream>
#include <cstring>
#include <string>
#include <unistd.h>
...
void RunPattern(LANetwork& net,  vector<double> pat, double label, int active_nrn_spike_thresh, float max_inp_freq, float label_inp_freq, bool Train)
{
	...
	for (i=0; i < 400; i++)//changed to MNIST vector size *INPUT DIM*
	{
	...
	if (tActive > 0)
	{
		net.Confidence = (float(tActiveSub[net.Prediction])/float(tActive));
	}
...
void LoadTrainingFiles(string tr, string tr_labs, string te, string te_labs)
{
	train_file = tr;
	test_file = te;

	train_labels_file = tr_labs;
	test_labels_file = te_labs;
}


/*
string train_file = "./Shape_Datasets/Shapes_train_0420";
string test_file = "./Shape_Datasets/Shapes_test_0420";

string train_labels_file = "./Shape_Datasets/Shapes_train_labels_0420";
string test_labels_file = "./Shape_Datasets/Shapes_train_labels_0420";
*/
...
vector<vector<double> > train_imgvec = read_csv("Blobs_train");

vector<vector<double>> templabs_train = read_csv("Blobs_train_labels");
vector<double> train_labelsvec = templabs_train[0];
...
vector<vector<double> > train_imgvec = read_csv(train_file);

vector<vector<double>> templabs_train = read_csv(train_labels_file);
vector<double> train_labelsvec = templabs_train[0];
...
vector<vector<double> > test_imgvec = read_csv("Blobs_test");

vector<vector<double>> templabs_test = read_csv("Blobs_test_labels");
vector<double> test_labelsvec = templabs_test[0];
...
vector<vector<double> > test_imgvec = read_csv(test_file);

vector<vector<double>> templabs_test = read_csv(test_labels_file);
vector<double> test_labelsvec = templabs_test[0];
...
/*
vector<vector<double> > val_imgvec = {test_imgvec.end() - 100, test_imgvec.end()};

vector<double> val_labelsvec = {test_labelsvec.end() - 100, test_labelsvec.end()};

test_imgvec = {test_imgvec.begin(), test_imgvec.end() - 100};

test_labelsvec = {test_labelsvec.begin(), test_labelsvec.end() - 100};
*/
...
//******************************************************
    ...
    int sample_size=100;//validation batch size 
    int samp_tot=0;//counter used for sample accuracy
    int sample_interval=20;//Amount of training iterations between sample tests 
    int purged = 6;// variable used to identify how many synapses where purged during turnover  RANDOM SYNAPSE TURNOVER
    int purged_1 = 6;// variable used to identify how many synapses where purged during turnover in subpop 1 CONSTRAINED SYNAPSE TURNOVER
    int purged_2 = 6;// variable used to identify how many synapses where purged during turnover in subpop 2 CONSTRAINED SYNAPSE TURNOVER
    //int purged_rec = 6; // variable used to identify how many synapses where purged during recurrent turnover RECURRENT RANDOM SYNAPSE TURNOVER
    //int purged_rec_1 = 6; // variable used to identify how many synapses where purged during recurrent turnover RECURRENT CONSTRAINED SYNAPSE TURNOVER
    //int purged_rec_2 = 6; // variable used to identify how many synapses where purged during recurrent turnover RECURRENT CONSTRAINED SYNAPSE TURNOVER
    ...
    //total synaptic turnover
    /*
    int tot_turnover_iter=40;
    int tot_purged = 6;
	*/
	...
	//ofstream recpyrweightsperepochcnt(("./results/recurrent_weights_per_epoch_"+suffix+".txt").c_str());
...
//********************************************************************************************************************************************************************
			...
			for (nrn_iter ni = net.pyr_list.begin(); ni != net.pyr_list.end(); ni++)
			{
				...
				for (syn_iter si = nrn->incoming.begin(); si!=nrn->incoming.end(); si++)
				{	
					if (find(net.pyr_list.begin(),net.pyr_list.end(),(*si)->source_nrn) != net.pyr_list.end())
					{
						recpyrweightsperepochcnt<<(*si)->weight<<"	";
					}
					
				}

			}
			recpyrweightsperepochcnt << endl;
			...
			*/
			...
			net.ResetSpikeCounters();
			cout << "Running " << k << " training pattern with label "<< train_labelsvec[i]<< endl;

			RunPattern(net, train_imgvec[i],train_labelsvec[i],active_neuron_spike_thresh, max_input_freq, label_input_freq, true);
			...
			*/
			...
			//****************RANDOM TOTAL SYNAPSE TURNOVER*****************
			
			/*
			if ((k % tot_turnover_iter)==0 && tot_purged > 5)//Total Synaptic Turnover
			{
				cout<<"Total Random Synaptic Turnover"<<endl;
				tot_purged = net.PurgeInputSynapses(100,1);
				int syns_kept = round(rewired_perc * tot_purged);
				net.ConnectNeurons(net.pattern_inputs, net.pyr_list, 0, syns_kept, 1, wmin, wmax, false, true);
			}
			*/
			...
			//****************RECURRENT RANDOM SYNAPSE TURNOVER*****************
			/*
			
			if ((k % turnover_iter)==0 && purged_rec > 5)//Recurrent Synaptic Turnover
			{
				cout<<"Recurrent Turnover"<<endl;
				purged_rec = net.PurgeRecurrentSynapsesNikos(wmax + turnover_thresh);
				int syns_kept = round(rewired_perc * purged_rec);
				net.ConnectNeurons(net.pyr_list, net.pyr_list, 0, syns_kept, 1, wmin, wmax, false, true);
			}
			...
			*/
			...
			//****************RECURRENT CONSTRAINED SYNAPSE TURNOVER*****************
			/*
			
			if ((k % turnover_iter)==0 && purged_rec_1 > 5)
			{
				cout<<"Recurrent Turnover for Subpopulation 1"<<endl;
				purged_rec_1 = net.PurgeRecurrentSynapsesInSubpopNikos(wmax + turnover_thresh, net.pyr_lists[class_1]);
				int syns_kept = round(rewired_perc * purged_rec_1);
				net.ConnectNeurons(net.pyr_lists[class_1], net.pyr_lists[class_1], 0, syns_kept, 1, wmin, wmax, false, true);
			}
			
			if ((k % turnover_iter)==0 && purged_rec_2 > 5)
			{
				cout<<"Recurrent Turnover for Subpopulation 2"<<endl;
				purged_rec_2 = net.PurgeRecurrentSynapsesInSubpopNikos(wmax + turnover_thresh, net.pyr_lists[class_2]);
				int syns_kept = round(rewired_perc * purged_rec_2);
				net.ConnectNeurons(net.pyr_lists[class_2], net.pyr_lists[class_2], 0, syns_kept, 1, wmin, wmax, false, true);
			}
...
//WEIGHT SAVING FILE(ALL WEIGHTS)
	...
	//*************************************************PREPOST+WEIGHT FILE************************************************************
	ofstream prepostcnt(("./results/prepost_w_"+suffix+".txt").c_str());
	...
	for (nrn_iter ni = net.pattern_inputs.begin(); ni != net.pattern_inputs.end(); ni++)
	{
		...
		for (syn_iter si = nrn->outgoing.begin(); si != nrn->outgoing.end(); si++)		
		{
			...
			prepostcnt << (nrn->nid - 120);
			prepostcnt <<"	" << s->target_branch->bid;
			prepostcnt <<"	" << s->weight << endl;
	...
	for (i=0; i < TOT_PAT_TEST; i++)
	{
		...
		*/
		...
		predictions<< test_labelsvec[i]<<" "<< net.Prediction << " " << correct << " " << net.Confidence << " " << net.totPyrActive << endl;
...
int main( int argc, char* argv[])
{
	...
	//Nikos Stuff
	int numPatternsTrain = 2000;//60000 for full mnist, 1006 for reduced centroid mnist, 2000 for blobs and shapes
	int numPatternsTest = 400; //10000 MNIST, 400 for blobs and shapes
	int epochs=1;
	int digit1 = 0;
	int digit2 = 1;
	...
	string training_file = "Shapes_train";
	string testing_file = "Shapes_test";
	string training_labels_file = "Shapes_train_labels";
	string testing_labels_file = "Shapes_test_labels";
	...
	while ((c = getopt(argc, argv, "M:N:E:e:t:H:B:I:i:P:p:T:S:s:d:w:O:g:l:b:c:o:t:xnLDRJCGhU"))!= -1)
	{
		switch (c)
		{
			case '?':
			...
			"-o Turnover_Type=[0,1,2]: Set the type of turnover. 0=Random, 1=Constrained, 2=No Turnover" << endl;
			...
			case 'T': training_file = strdup(optarg); break;
			case 't': training_labels_file = strdup(optarg); break;
			case 'E': testing_file = strdup(optarg); break;
			case 'e': testing_labels_file = strdup(optarg); break;
			...
			case 'o': 
				...
				if (o)
				{
					*o = '\0';
					...
					else if (!strcmp(optarg,  "difficulty_synapses")) difficulty_synapses = atof(val);
	...
	if (load==0)
	{
		...
		net.CreateFearNet(nneurons, nbranches, inh_control_synapses, pattern_to_hidden_synapses, label_to_hidden_synapses, difficulty_synapses, lateral_inh_synapses, Wmin, Wmax, digit1, digit2);
	}
	...
	LoadTrainingFiles(training_file,training_labels_file,testing_file,testing_labels_file);
