...
struct LASynapse
{
	...
	float weight, iltp, eltp, pos, calcium, stag, trigger, ltag, stdpTag, syn_learning_rate ;
...
class LANetwork
{
	...
	//Lateral Inhibition Additions
	vector<LANeuron*> in_pv_0;
	vector<LANeuron*> in_pv_1;

	//Label input subpopulations(one per digit class)
	vector<LANeuron*> label_inputs_0;
	vector<LANeuron*> label_inputs_1;
	vector<LANeuron*> label_inputs_2;
	vector<LANeuron*> label_inputs_3;
	vector<LANeuron*> label_inputs_4;
	vector<LANeuron*> label_inputs_5;
	vector<LANeuron*> label_inputs_6;
	vector<LANeuron*> label_inputs_7;
	vector<LANeuron*> label_inputs_8;
	vector<LANeuron*> label_inputs_9;

	vector< vector<LANeuron*> > label_inputs{ label_inputs_0, label_inputs_1, label_inputs_2, label_inputs_3, label_inputs_4, label_inputs_5, label_inputs_6, label_inputs_7, label_inputs_8, label_inputs_9 };

	//Pyramidal subpopulations(one per digit class)
	vector<LANeuron*> pyr_list_0;
	vector<LANeuron*> pyr_list_1;
	vector<LANeuron*> pyr_list_2;
	vector<LANeuron*> pyr_list_3;
	vector<LANeuron*> pyr_list_4;
	vector<LANeuron*> pyr_list_5;
	vector<LANeuron*> pyr_list_6;
	vector<LANeuron*> pyr_list_7;
	vector<LANeuron*> pyr_list_8;
	vector<LANeuron*> pyr_list_9;

	vector< vector<LANeuron*> > pyr_lists{ pyr_list_0, pyr_list_1, pyr_list_2, pyr_list_3, pyr_list_4, pyr_list_5, pyr_list_6, pyr_list_7, pyr_list_8, pyr_list_9 };
	...
	int nlTypePV, nlTypeSOM, nlTypePYR;
	...
	//More Nikos Additions
	float totPyrActiveSub[10];
	int Prediction;
	float Confidence;
	...
	void CreateFearNet(int, int, float, float, float, float, float, float, float, int, int);
...
;
	...
	int ConnectNeurons(vector<LANeuron*> fromList, vector<LANeuron*> toList, bool isClustered,  int nNeuronPairs, int nSynapsesPerNeuron, float initialWeightMin, float initialWeightMax, bool fullCon, bool isPlastic);
	...
	int PurgeInputSynapses( int totalToRemove, float weightLimit);

	// Remodeled by Nikos
	int PurgeInputSynapsesNikos( float weightLimit);

	// Remodeled by Nikos
	int PurgeRecurrentSynapsesNikos( float weightLimit);

	int PurgeRecurrentSynapsesInSubpopNikos( float weightLimit, vector<LANeuron*> subpop);

	// Changed for keeping track of turnover in specific subpops
	int PurgeInputSynapsesInSubpopNikos( float weightLimit, vector<LANeuron*> subpop);
	...
	void CreateNeurons(int number, int n_branches_per_neuron, char type, vector<LANeuron*>* appendTo, vector<LANeuron*>* appendTo2, int inputId);

	//Load existing network from file(nikos)
	void LoadNet(int nneurons, int nbranches,  int class_1, int class_2, string filename);
